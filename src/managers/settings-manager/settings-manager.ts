...
export class SettingsManager extends BaseManager implements ISettingsManager {
    ...
    protected async initialize(): Promise<void> {
        const setting = await AsyncStorage.getItem(SettingsConfiguration.pushNotifications.joinRequestNotifications.key);
        this._joinRequestNotificationsAllowed$.next(
            setting === null ? SettingsConfiguration.pushNotifications.joinRequestNotifications.default : setting === "true"
        );
    }    
    ...
    async setJoinRequestNotificationsAllowed(allowed: boolean): Promise<void> {
        try {
            await AsyncStorage.setItem(SettingsConfiguration.pushNotifications.joinRequestNotifications.key, `${allowed}`.toLowerCase());
