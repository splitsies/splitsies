...
export class ExpenseMessageStrategy implements IExpenseMessageStrategy {
    ...
    private async processItemSelection(message: ExpenseMessage, current: IExpenseDto): Promise<IExpenseDto> {
        ...
        const item = [...current.items, ...current.children.flatMap(c => c.items)]
            .find(i => i.id === message.itemSelectionUpdate!.itemId);
    ...
    private processItemDetails(message: ExpenseMessage, current: IExpenseDto): IExpenseDto {
        ...
        const itemIndex = current.children[childIndex].items.findIndex(i => i.id === message.itemDetailsUpdate?.item.id);
        if (itemIndex === -1) return current;

        
        const mergedItem = { ...message.itemDetailsUpdate.item, owners: current.children[childIndex].items[itemIndex].owners };
