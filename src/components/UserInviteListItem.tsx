...
export const UserInviteListItem = ({ user, contactUsers, expenseUsers, pendingUserIds, onInviteUser }: Props): JSX.Element => {
    
    const computeButtonLabel = useCallback((user: IExpenseUserDetails) => {
                ...
                <View style={styles.buttonContainer}>
                    {!expenseUsers.map(u => u.id).includes(user.id) && !!user.phoneNumber &&
                        <Chip
                            ...
                            labelStyle={[styles.buttonLabel, { color: pendingUserIds.includes(user.id) ? _colorConfiguration.greyFont : _colorConfiguration.black }]}
                            containerStyle={{ width: 120, borderColor: pendingUserIds.includes(user.id) ? _colorConfiguration.primaryTranslucentLight : _colorConfiguration.primary }}
                            backgroundColor={pendingUserIds.includes(user.id) ? _colorConfiguration.primaryTranslucentLight : _colorConfiguration.primary}
                            label={computeButtonLabel(user)}
                            onPress={() => onInviteUser(user)}
                        />
                    }

                    {expenseUsers.map(u => u.id).includes(user.id) && <Icon assetName="checkCircle" size={30} tintColor={_colorConfiguration.primary} />}
                </View>
            </View>
        </View>
    )
